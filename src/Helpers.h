...
bool AddImage( const ofBaseHasTexture& hasTexture, const ofVec2f& size );
bool AddImage( const ofTexture& texture, const ofVec2f& size );
void ImageTooltip(const ofTexture & texture, const std::string& text = "", const ofVec2f & peekSize = {200.f,200.f}, float peekScale = 1.f);
}  // namespace ofxImGui
